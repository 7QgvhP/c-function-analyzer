...
解決できたメンバは、**型だけでなく定義位置もそのメンバのもの**になります。「定義へ」ボタンは構造体変数の宣言ではなく、メンバの宣言行（インクルード先のヘッダを含む）へジャンプします。

構造体定義やメンバが見つからない場合は、根元の型で代用せず**型を `(推定)`、定義位置を未設定**にします。誤った型や無関係な場所へのジャンプを提示するより、解決できていないことを明示する方が誤解を招かないためです。

| 状況 | 型 | 定義位置 |
|---|---|---|
| メンバまで解決できた | メンバの型 | **メンバの宣言行** |
| メンバが見つからない | `(推定)` | **なし**（ボタンを表示しない） |
| 構造体定義が見つからない | `(推定)` | **なし** |
| メンバを伴わない変数 | 変数の型 | 変数の宣言行 |

ポインタ引数のメンバアクセス（`data->id`）にも同じ解決を適用します。引数そのもの（`*out = 1` の `out`）は従来どおり引数の宣言行を指します。
...
定義値は AST 上で**行末までの生テキスト**として得られるため、末尾に書かれた行コメントが含まれます。そのため `splitMacroValue` で値とコメントに分けます。文字列リテラル・文字リテラルの内外を判定しながら走査するため、`#define URL "http://example.com"` の `//` はコメントとして扱いません。

コメントの取得方法は、書き方によって異なります。

| 書き方 | AST 上の位置 | 取得方法 |
|---|---|---|
| `#define LIMIT 100 /* 上限値 */` | 独立した `comment` ノード | `findTrailingComment` |
| `#define LIMIT 100 // 上限値` | `preproc_arg`（値）の中 | `splitMacroValue` |
| `#define ENABLED // 有効化` | 独立した `comment` ノード（`preproc_arg` が作られない） | `findTrailingComment` |

行コメントは独立したノードにならないため、値の解析時に取り出します。両方が取れた場合は、値から取り出した側を優先します。

なお、小文字を含むマクロは大文字マクロ分類の対象外となり「マクロ変数」ではなく入出力変数として分類されますが、変数宣言が見つからない場合はマクロ定義を参照して定義値と定義位置を表示します。

マクロ変数は「型名 → 定義名 → 定義値」の3列で表示します。定義値は型名には含めず、`VariableInfo.value` として別の欄に出します。

| 状態 | 型名欄 | 定義値欄 |
|---|---|---|
| 値を持つマクロ | `macro` | 定義値（例: `10`） |
| 値を持たないマクロ（`#define ENABLED`） | `macro` | 空欄 |
| 定義が見つからない | `(推定)` | 空欄 |
| `enum` の列挙子 | `enum` | 列挙子の値 |

定義値には別のマクロや変数が書かれていることがあるため、表示幅は名前より優先して確保します。収まらない場合は末尾を省略し、全文は `title` 属性（ホバー）で参照できます。

#### enum 列挙子の収集 (`collectEnumConstants`)

`enum` の列挙子は `#define` と同じく「名前に値が結び付いた定数」であるため、同じマップ（`FileScopeSymbols.macros`）へ登録し、マクロ変数として表示します。型名欄は `enum` として `#define` 由来と区別します（`MacroDefinition.kind`）。
...
- 明示された値は**記述されたまま**表示し、計算した値のみ10進で表示します
- 直前の値が整数リテラル（10進・16進・8進、型サフィックス付きも可）として解釈できない場合は、式に加算量を付けた形にします
- 先頭から値が省略された場合に 0 から始まるよう、直前の値の初期値を `-1` としています
- 走査は `forEachFileScopeNode` + `walk` で行うため、`#ifdef` の内側や `typedef enum { ... } Alias;` の形も拾えます
- 関数ボディ内で定義されたローカルな `enum` は対象外です
- 同名の `#define` が既に登録されている場合は上書きしません（プリプロセッサが先に展開するため）

### 宣言のコメントの収集 (`findTrailingComment`)

宣言の右側に書かれたコメントを収集し、専用の欄へ表示します。組込みコードでは変数の説明を宣言の右に書く習慣が広くあるためです。

コメントは AST 上で**宣言ノードの後続の兄弟**として現れます（`#define` の場合のみ `preproc_def` の子）。

```
declaration      |extern int g_count;|
comment          |/* 実行回数 */|        ← 兄弟として現れる

field_declaration_list
  field_declaration  |U8  mode;|
  comment            |/* 動作モード */|   ← 同上
```

そのため次の手順で探します。

1. 宣言位置のノードから、**同じ行にある**後続の兄弟を辿り `comment` を探す
2. 見つからなければ、**同じ行で終わる親**へ一段上がって 1 を繰り返す
3. 行が変わった時点、または親が別の行で終わる時点で打ち切る

3 の条件により、次の行に書かれたコメントを誤って拾うことはありません。また関数定義のように親が離れた行で終わる場合も、関数末尾のコメントを拾いません。

収集対象は変数宣言（`collectDeclaredVars`）、構造体メンバ、引数（`parseSignature`）、マクロ（`collectMacros`）、`enum` の列挙子（`collectEnumConstants`）、関数（`collectFileScopeFunctions`）です。呼び出し関数では、宣言側のコメントをマクロ定義側より優先します。

コメント本文は `normalizeComment` で記号と連続する空白を取り除いてから表示します。通常のコメント記号に加え、Doxygen 形式の目印（`!` と、直前の要素を指す `<`）も対象です。
...
置換を重ねる方式では、記号だけのコメント（`/**/`）で記号が残るため、開始記号と終端記号で挟まれた**本文を正規表現で取り出す**方式にしています。

構造体メンバのコメントは、型・定義位置と同様に `resolveAccessPath` を通じて最終メンバのものが使われます。
...
| 除外 | 設定 `excludePaths` に該当する場所は配下ごと対象外（再帰展開でも辿りません） |
...
候補パスの組み立ては `src/includePaths.ts` の純関数に分離しており、ファイルシステムや VS Code API に触れずにテストできます。候補は「インクルード元ファイルのディレクトリ」「各ワークスペースフォルダの直下」の2つで、同一パスが複数回現れた場合は重複を除去します。

ここで1件も見つからない場合はファイル名検索へフォールバックするため、`src/` と `include/` が分かれているような構成でも設定は不要です。

```
proj/
├─ src/main.c        →  #include "types.h"
└─ include/types.h        ファイル名検索で解決される
```

探索範囲はワークスペース内に限られます。ワークスペース外のヘッダ（外部SDKなど）は解決できません。

設定値（`excludePaths` など）は解決のたびに読み取るため、変更は再起動なしで反映されます。

#### ファイルの文字コード判別 (`decodeSource`)

インクルードファイルは `fs.readFileSync` でバイト列として読み、内容から文字コードを判別してデコードします。UTF-8 固定で読むと、Shift-JIS で保存されたヘッダのコメント（日本語）が文字化けするためです。

```
Shift-JIS の「動作モード」を UTF-8 として読むと  →  ???????[??
```

| 順 | 判定 | 備考 |
|---|---|---|
| 1 | バイト順マーク（BOM） | UTF-8 / UTF-16 LE / UTF-16 BE。BOM は本文から取り除く |
| 2 | UTF-8 として妥当か | `TextDecoder('utf-8', { fatal: true })` が例外を投げなければ UTF-8。ASCII のみのファイルもここに該当 |
| 3 | 上記以外 | 設定 `fallbackEncoding` の文字コード（既定は `shift_jis`） |

Shift-JIS の日本語を含むバイト列は UTF-8 として不正になるため、2 の判定で確実に切り分けられます。実行環境が対応していない文字コード名を指定された場合は、例外にせず UTF-8 で代用します。

解析対象のファイル自身は `document.getText()`（VS Code が判別した内容）を使うため、この処理を通りません。

#### 探索対象からの除外 (`buildExcludeRules` / `isExcluded`)

設定 `excludePaths` は、記述形式によって2通りの判定を使い分けます。判定は `buildIncludeCandidates` とファイル名検索の双方に適用されます。

| 形式 | 判定条件 | 判定内容 |
|---|---|---|
| ディレクトリ指定 | 絶対パス、または `/` `\` を含む | 解決した絶対パスの配下かどうか（`isUnderAny`） |
| フォルダ名指定 | 上記以外 | ワークスペース相対パスの各フォルダ名に、指定文字列が含まれるか（`isUnderExcludedFolderName`） |

フォルダ名指定の判定には次の制約があります。

- 判定範囲は**ワークスペースフォルダからの相対部分のみ**です。`C:\Users\old_user\...` のようなワークスペース外のパスや、ワークスペースフォルダ自身の名前には巻き込まれません
- **ファイル名は判定対象に含めません**（`path.dirname` を取ってから各セグメントを判定）。フォルダのみを対象とするためです
- 大文字・小文字を区別しません（Windows のファイルシステムに合わせるため）
- **部分一致**です。ちょうど1つのフォルダのみを除外したい場合は `./variantB` のように区切り文字を付けてディレクトリ指定にします
...
| 定義が見つからない `UNKNOWN_LIMIT` | マクロ（推定） | `(推定)` |
